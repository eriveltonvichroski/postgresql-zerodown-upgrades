...
function setup_pgpool(){
	...
	rm -f /etc/pgpool2/*
	cp /vagrant/pgpool/* /etc/pgpool2/
	cat /vagrant/pgpool/.pcppass > /etc/pgpool2/.pcppass
	cat /vagrant/pgpool/pgpool-"$(hostname)".conf > /etc/pgpool2/pgpool.conf
	chmod 600 /etc/pgpool2/.pcppass
	mkdir -p /var/run/pgpool/ /var/log/pgpool/
	chown -R postgres.postgres /var/run/pgpool/ /etc/pgpool2/ /var/log/pgpool/
	mkdir -p /var/run/postgresql/ /var/log/postgresql/
	chown -R postgres.postgres /var/run/postgresql/ /var/log/postgresql/
	chmod 777 -R /var/run/postgresql/ /var/log/postgresql/
...
function setup_fresh_postgresql() {
    ...
    su - postgres -c "/usr/lib/postgresql/${POSTGRESQL_VERSION}/bin/psql -c \"ALTER USER postgres  WITH PASSWORD 'postgres';\"" 2>> /tmp/x 1>> /tmp/x
    su - postgres -c "/usr/lib/postgresql/${POSTGRESQL_VERSION}/bin/psql -c \"create extension pgagent;\"" 2>> /tmp/x 1>> /tmp/x
...
function setup_postgresql() {
    ...
    apt-get -y install postgresql-${POSTGRESQL_VERSION} pgagent
...
primitive PgBouncerVIP ocf:heartbeat:IPaddr2 \
params ip=172.28.33.9 cidr_netmask=32 op monitor interval=5s \
meta resource-stickiness=200

primitive PostgresqlVIP ocf:heartbeat:IPaddr2 \
params ip=172.28.33.10 cidr_netmask=32 op monitor interval=5s

primitive pgpool ocf:heartbeat:pgpool \
  params pgpoolconf="/etc/pgpool2/pgpool.conf" \
	   	pcpconf="/etc/pgpool2/pcp.conf" \
	   	options="-D"  pgpooluser="postgres" \
	   	checkmethod="pcp" \ 
		checkstring="-w -h localhost -p 9898" \
	   	pgpoolcmd="/usr/sbin/pgpool" \
	   	pcpnccmd="/usr/sbin/pcp_node_count" \
	   	pcpnicmd="/usr/sbin/pcp_node_info" \
	   	pcpatcmd="/usr/sbin/pcp_attach_node" \
	   	dsocketdir="/var/run/postgresql" \
	   	portpgpool="5433" \
	   	portpcp="9898" \
	   	pcppass="/etc/pgpool2/.pcppass" \
  op start   interval=0s  timeout=60s \
  op monitor interval=30s timeout=30s \
  op stop    interval=0s  timeout=60s

primitive pgagent ocf:heartbeat:pgagent \
  params connection_string="user=postgres host=/var/run/postgresql dbname=postgres" \
	   options="-r 1 -t 1"  \
	   user="postgres" \
  op start   interval=0s  timeout=60s \
  op monitor interval=30s timeout=30s \
...
primitive Postgresql ocf:heartbeat:pgsql \
    params pgctl="/usr/lib/postgresql/${POSTGRESQL_VERSION}/bin/pg_ctl" \
    psql="/usr/bin/psql" \
    pgdata="/var/lib/postgresql/${POSTGRESQL_VERSION}/main/" \
    start_opt="-p 5432" \
    rep_mode="sync" \
    node_list="pg01 pg02 pg03" \
    primary_conninfo_opt="keepalives_idle=60 \
    keepalives_interval=5 \
    keepalives_count=5" \
    master_ip="172.28.33.10" \
    repuser="postgres" \
    tmpdir="/var/lib/postgresql/${POSTGRESQL_VERSION}/tmp" \
    config="/etc/postgresql/${POSTGRESQL_VERSION}/main/postgresql.conf" \
    logfile="/var/log/postgresql/postgresql-crm.log" \
...
params master-max=1 master-node-max=1 clone-max=3 clone-node-max=1 notify=true

colocation pgbouncer-vip-prefers-master 100: PgBouncerVIP msPostgresql:Master
colocation pgpool-with-ip INFINITY: pgpool msPostgresql:Master
colocation pgagent-with-ip INFINITY: pgagent msPostgresql:Master
...
order pgpool-after-ip mandatory: PostgresqlVIP pgpool 
order pgagent-after-ip mandatory: PostgresqlVIP pgagent
